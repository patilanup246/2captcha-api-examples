...
function myscandir($dir, $sort=0)
{
    ...
    // remove unnecessary dots (rarely used)
...
$limit = '5'; /// Limit simultaneous uploads
if($thread > $limit){
    $thread = $limit;
}
...
do{
    ...
    // Captchas in progress
    if(!empty($captcha_ids)){
        foreach($captcha_ids as $captchaid => $value){
            $answer = file_get_contents("http://2captcha.com/res.php?action=get&id=$captchaid&key=$key");
    ...
    if(count($captcha_ids) < $limit && !empty($files)){
        ...
        foreach($files_slice as $_key => $img){        
            ...
            $urls[$_key] = 'http://2captcha.com/in.php';
        }
